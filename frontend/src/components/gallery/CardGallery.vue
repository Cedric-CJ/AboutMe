...
export default {
  ...
  computed: {
    ...
    localizedCards() {
      return this.cards.map(card => ({ ...card, caption: this.language === 'en' ? card.captionEN : card.captionDE }))
    }
  }
}
</script>
<style>
.CardGallery { display:flex; justify-content:center; align-items:center; margin:10vmin; overflow:hidden; transform:skew(5deg); }
.CardGallery .card { flex:1; transition:all 1s ease-in-out; height:75vmin; position:relative; }
.CardGallery .card .card__head { color:black; background:rgba(255,30,173,0.75); padding:.5em; transform:rotate(-90deg); transform-origin:0% 0%; transition:all .5s; min-width:100%; text-align:center; position:absolute; bottom:0; left:0; font-size:1em; white-space:nowrap; }
.CardGallery .card:hover { flex-grow:10; }
.CardGallery .card:hover .card__image { filter:grayscale(0); }
.CardGallery .card:hover .card__head { text-align:center; top:calc(100% - 2em); color:white; background:rgba(0,0,0,0.5); font-size:2em; transform:rotate(0deg) skew(-5deg); }
