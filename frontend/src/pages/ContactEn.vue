<template>
  <section class="page-auto-contrast min-h-[70vh] flex flex-col items-center justify-center">
    <h2 class="text-white text-3xl font-semibold mb-8">Contact</h2>
    ...
    <div class="glass-card mt-8 w-full max-w-2xl p-5">
      <h3 class="text-white text-xl font-semibold mb-4">Send an inquiry</h3>
      <form @submit.prevent="submitInquiry" class="grid grid-cols-1 gap-3">
        ...
        <input v-model="form.subject" type="text" placeholder="Subject" class="input" required />
        <textarea v-model="form.message" placeholder="Message" class="input min-h-32" required></textarea>
        <div class="flex gap-2 justify-end">
          <button type="reset" class="glass-btn-secondary px-4 py-2 rounded" @click="resetForm">Reset</button>
          <button type="submit" class="glass-btn px-4 py-2 rounded">Send</button>
        </div>
      </form>
    </div>
  </section>
...
function openEmail(){ window.location.href = 'mailto:info@specialcode.de' }
...
function submitInquiry(){ console.log('Inquiry:', form.value) }
