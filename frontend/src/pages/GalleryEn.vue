<template>
  <div class="relative">
    ...
    <section class="relative z-10 px-4 max-w-6xl mx-auto">
      <h2 class="text-white text-xl mb-4">Animated</h2>
      <div class="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        <div class="glass-card overflow-hidden cursor-pointer" @click="open('Slide')">
          <div class="h-40 bg-cover bg-center" :style="bg('slide')" />
          <div class="p-3 text-white">
            <h3 class="font-medium">Sliding City Gallery</h3>
            <p class="text-sm text-zinc-300">Beautiful city scenes with info panels</p>
          </div>
        </div>
        <div class="glass-card overflow-hidden cursor-pointer" @click="open('Card')">
          <div class="h-40 bg-cover bg-center" :style="bg('card')" />
          <div class="p-3 text-white">
            <h3 class="font-medium">Card Gallery</h3>
            <p class="text-sm text-zinc-300">Tilted cards with stylish hover effects</p>
          </div>
        </div>
        <div class="glass-card overflow-hidden cursor-pointer" @click="open('Grid')">
          <div class="h-40 bg-cover bg-center" :style="bg('grid')" />
          <div class="p-3 text-white">
            <h3 class="font-medium">Grid Gallery</h3>
            <p class="text-sm text-zinc-300">Interactive 3x3 grid</p>
          </div>
        </div>
        <div class="glass-card overflow-hidden cursor-pointer" @click="open('Box')">
          <div class="h-40 bg-cover bg-center" :style="bg('box')" />
          <div class="p-3 text-white">
            <h3 class="font-medium">Box Gallery</h3>
            <p class="text-sm text-zinc-300">Flexible boxes with zoom effect</p>
          </div>
        </div>
      </div>
    </section>
