...
export default {
  ...
  methods: {
    ...
    // Hintergrund-Animation (ohne Scroll-Logik)
    initBackground() {
      let width, height, largeHeader, canvas, ctx, points, target;
      let stars = []; // Array für Sterne
      let shootingStars = []; // Array für aktive Sternschnuppen
      ...
      function initHeader() {
        ...
        // Punkte für den interaktiven Hintergrund erstellen
        points = [];
        ...
        // Für jeden Punkt die 5 nächsten Punkte finden
        points.forEach(p1 => {
          const closest = [];
          points.forEach(p2 => {
            if (p1 !== p2) {
        ...
        // Jedem Punkt einen Kreis zuweisen
        points.forEach(p => {
          p.circle = new Circle(p, 2 + Math.random() * 2, 'rgba(218,165,32,0.3)');
        });
      }

      // Sterne initialisieren (mit zufälliger Twinkle-Amplitude und -Phase)
      function initStars() {
        const numStars = 100;
        stars = [];
        for (let i = 0; i < numStars; i++) {
          stars.push({
            ...
            radius: 0, // wird gleich initialisiert
      ...
      // Sternschnuppen updaten und zeichnen
      ...
      // Sterne zeichnen (ohne Scroll-Offset)
      function drawStars() {
        if (!ctx) return;
        stars.forEach(star => {
          const x = star.x;
          const y = star.y; // Keine Verschiebung durch Scroll
          const newAlpha = star.baseAlpha + star.twinkleAmplitude * Math.sin(performance.now() * star.twinkleSpeed + star.phase);
      ...
      // Tween-Funktion ohne externe Bibliotheken
      ...
      // Kreis-Konstruktor für interaktive Punkte
      ...
      // Initialisierung und Animation starten
      initHeader();
      addListeners();
      initAnimation();
    }
